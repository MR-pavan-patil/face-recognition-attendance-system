...
def today_attendance():
    ...
    # Teacher: own dept only
    if hasattr(g, 'user') and g.user and g.user.is_teacher and g.user.department:
        query = query.filter(Student.department == g.user.department)
    if search:
    ...
    st_q = Student.query.filter_by(is_active=True)
    if hasattr(g, 'user') and g.user and g.user.is_teacher and g.user.department:
        st_q = st_q.filter_by(department=g.user.department)
    total   = st_q.count()
    present_q = db.session.query(Attendance).join(Student).filter(Attendance.date == today, Attendance.status == 'Present')
    if hasattr(g, 'user') and g.user and g.user.is_teacher and g.user.department:
        present_q = present_q.filter(Student.department == g.user.department)
    present = present_q.count()
